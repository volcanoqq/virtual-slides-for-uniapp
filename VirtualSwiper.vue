<template>
  <view class="swiper-limit-load-container">
    ...
    <swiper
      ...
      @touchmove.stop
    >
      <swiper-item v-for="(item, index) in renderList" :key="item.ID">
...
const START = 0; //开始下标
const END = 2; //结束下标
const SWIPER_LENGTH = 3; //Swiper长度
const NO_PREV_PAGE = -1; //上临界
const NO_NEXT_PAGE = -2; //下临界
...
export default {
  ...
  mounted() {
    this.previousIndex = this.current % SWIPER_LENGTH;
  },
  watch: {
    /**
     * 侦听 current 的改变
  ...
  methods: {
    ...
    initSwiperList(allDataList, contentIndex) {
      ...
      renderList[this.getLastVisibleIndex(visibleIndex)] =
        this.getLastRenderItem(allDataList, contentIndex);
      renderList[this.getNextVisibleIndex(visibleIndex)] =
    ...
    getNextRenderItem(allDataList, contentIndex) {
      ...
      return nextContentIndex === this.total
        ? null
        : allDataList[nextContentIndex];
    },
    ...
    swiperChangeEvent(e, externalCall) {
      ...
      const contentIndex = allDataList.findIndex(
